...
# ---------------------------- 重写：仪表盘 ----------------------------
def show_dashboard():
    st.markdown('<h2 class="sub-header">📈 数据驾驶舱</h2>', unsafe_allow_html=True)
    ...
    for col, (label, value, unit) in zip(cols, metrics_invest):
        with col:
            st.markdown(f"""
                <div class="stat-card">
...
def main():
    ...
    with st.sidebar:
        ...
        if not st.session_state.logged_in:
            # 未登录时侧边栏只展示系统信息
            st.markdown("## 🏢 一中心一基地")
            st.markdown("湖南数据管理系统")
            st.markdown("---")
            st.markdown("请使用右侧登录窗口")
        else:
            current_user = st.session_state.current_user
