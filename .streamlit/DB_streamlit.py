...
class EnterpriseSupportSystem:
    ...
    def init_database(self):
        ...
        # 检查示例表单是否存在
        cursor.execute("SELECT COUNT(*) FROM form_definitions WHERE form_name='信息总台账'")
        if cursor.fetchone()['count'] == 0:
            # 创建示例表单
            example_form_config = {
                "form_name": "信息总台账",
            ...
            cursor.execute("INSERT INTO form_definitions (form_name, form_config) VALUES (%s, %s) RETURNING id",
                           ("信息总台账", json.dumps(example_form_config)))
...
def show_form_management():
    ...
    with tab2:
        ...
        with st.form("create_main_form"):
            form_name = st.text_input("表单名称", key="form_name_input", placeholder="例如: 信息总台账")
