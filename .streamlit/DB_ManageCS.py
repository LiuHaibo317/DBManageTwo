...
class EnterpriseSupportSystem:
    ...
    ROLE_PERMISSIONS = {
        ...
        "manager": ["view_dashboard", "view_data", "add_data", "edit_data",
                    ...
                    "view_files", "manage_files"],  # 经理可管理文件
        "user": ["view_dashboard", "view_data", "add_data", "edit_own_data", "view_files"],  # 用户可查看文件
        "viewer": ["view_dashboard", "view_data", "view_forms", "view_reports", "view_files"]  # 只读用户可查看文件
    }
    ...
    def init_database(self):
        ...
        if cursor.fetchone()[0] == 0:
            ...
            # 初始化默认角色权限
            default_roles = [
                ("admin", "系统管理员", json.dumps(["all"], ensure_ascii=False)),
                ("manager", "部门经理", json.dumps([
                    "view_dashboard", "view_data", "add_data", "edit_data",
                    "export_data", "view_forms", "view_reports", "export_report",
                ...
                ("viewer", "只读用户", json.dumps([
                    "view_dashboard", "view_data", "view_forms", "view_reports",
                    "view_files"
                ], ensure_ascii=False))
            ]
            ...
            for role_name, description, permissions in default_roles:
                cursor.execute('''
                    INSERT OR IGNORE INTO role_permissions (role_name, description, permissions) 
                    VALUES (?, ?, ?)
        ...
        if cursor.fetchone()[0] == 0:
            ...
            cursor.execute("INSERT INTO form_definitions (form_name, form_config) VALUES (?, ?)",
                           ("帮扶总台账", json.dumps(example_form_config, ensure_ascii=False)))
            ...
            for data in example_data:
                cursor.execute("INSERT INTO form_data (form_id, data_json) VALUES (?, ?)",
                               (form_id, json.dumps(data, ensure_ascii=False)))
    ...
    def login(self, username, password, ip_address=None, user_agent=None):
        ...
        try:
            ...
            if user:
                if 'is_active' in column_names and len(user) == 4:
    ...
    def has_permission(self, user_id, permission):
        ...
        role, user_perms_json, role_perms_json = result

        # 检查用户特定权限
        if user_perms_json:
            try:
                user_perms = json.loads(user_perms_json)
        ...
        if role_perms_json:
            try:
                role_perms = json.loads(role_perms_json)
    ...
    def log_operation(self, user_id, username, operation, target_type=None, target_id=None, details=None,
                      ip_address=None):
        """记录用户操作日志"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
    ...
    def delete_user(self, user_id):
        ...
        cursor.execute('SELECT COUNT(*) FROM users WHERE role = ? AND id != ?', ("admin", user_id))
    ...
    def update_role_permissions(self, role_name, permissions, description=None):
        ...
        if description:
            cursor.execute('''
                   ...
                   WHERE role_name = ?
               ''', (json.dumps(permissions, ensure_ascii=False), description, role_name))
        else:
            cursor.execute('''
                   UPDATE role_permissions 
                   SET permissions = ?
                   WHERE role_name = ?
               ''', (json.dumps(permissions, ensure_ascii=False), role_name))
    ...
    def get_operation_logs(self, limit=100, user_id=None, operation=None):
        ...
        if operation:
            conditions.append("ol.operation = ?")
            params.append(operation)

        if conditions:
            query += " WHERE " + " AND ".join(conditions)

        query += " ORDER BY ol.operation_time DESC LIMIT ?"
        params.append(limit)
    ...
    def get_form_data(self, form_id):
        ...
        # 获取表单定义
        cursor.execute("SELECT form_config FROM form_definitions WHERE id=?", (form_id,))
        result = cursor.fetchone()
        ...
        form_config = json.loads(result[0])
        fields = form_config.get("fields", [])

        # 获取数据
        cursor.execute("SELECT id, data_json FROM form_data WHERE form_id=?", (form_id,))
    ...
    def save_form_data(self, form_id, df):
        ...
        for _, row in df.iterrows():
            ...
            cursor.execute("INSERT INTO form_data (form_id, data_json) VALUES (?, ?)",
                           (form_id, json.dumps(record_data, ensure_ascii=False)))
    ...
    def _save_file_bytes(self, original_filename, file_bytes, uploaded_by, description="", form_id=None, record_id=None):
        """
        内部方法：将文件字节保存到磁盘并插入数据库记录
        :param original_filename: 原始文件名（可包含路径）
        :param file_bytes: 文件二进制数据
        :param uploaded_by: 上传用户ID
        :param description: 描述
        :param form_id: 关联表单ID（可选）
        :param record_id: 关联记录ID（可选）
        :return: 文件ID
        """
        # 生成存储文件名（保留原始文件名的扩展名，加入日期和UUID）
        ...
        cursor.execute('''
            INSERT INTO files 
            (filename, original_filename, file_path, file_size, mime_type, uploaded_by, description, form_id, record_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (stored_filename, original_filename, file_path, file_size, mime_type,
              uploaded_by, description, form_id, record_id))
        ...
        self.log_operation(
            user_id=uploaded_by,
            username=st.session_state.get('current_user', {}).get('username', 'system'),
    ...
    # ---------- 文件管理相关方法 ----------
    def upload_file(self, uploaded_file, uploaded_by, description="", form_id=None, record_id=None):
        """
        上传单个文件（兼容Streamlit UploadedFile对象）
        """
        if uploaded_file is None:
            return None
        file_bytes = uploaded_file.getbuffer()
        return self._save_file_bytes(uploaded_file.name, file_bytes, uploaded_by, description, form_id, record_id)
    ...
    def upload_archive(self, archive_file, uploaded_by, description="", form_id=None, record_id=None):
        # ... 省略解压逻辑 ...
        file_id = self._save_file_bytes(
            original_filename=original_name,
            file_bytes=file_bytes,
            uploaded_by=uploaded_by,
            description=f"{description} (来自压缩包)" if description else "来自压缩包",
            form_id=form_id,
            record_id=record_id
        )
        # ...

    def upload_folder_zip(self, zip_file, uploaded_by, description="", form_id=None, record_id=None):
        """
        上传ZIP压缩包，解压后保存其中所有文件
        :param zip_file: Streamlit UploadedFile 对象（ZIP格式）
        :param uploaded_by: 上传用户ID
        :param description: 文件夹描述（将应用于所有文件？此处仅作为整体描述，不单独设置）
        :param form_id: 关联表单ID（可选）
        :param record_id: 关联记录ID（可选）
        :return: 成功上传的文件数量，以及文件ID列表
        """
        if zip_file is None:
            return 0, []

        # 读取ZIP文件到内存
        zip_bytes = io.BytesIO(zip_file.getbuffer())
        success_count = 0
        file_ids = []
        ...
        try:
            with zipfile.ZipFile(zip_bytes, 'r') as zf:
                ...
                for file_info in zf.infolist():
                    ...
                    # 保存文件（使用原始文件名中的路径信息）
                    # 注意：如果文件很多，可能会慢，可考虑批量插入，但此处简单处理
                    file_id = self._save_file_bytes(
                        original_filename=original_name,
                        file_bytes=file_bytes,
                        uploaded_by=uploaded_by,
                        description=f"{description} (来自文件夹)" if description else "来自文件夹",
                        form_id=form_id,
                        record_id=record_id
                    )
                    if file_id:
            ...
            return success_count, file_ids
        except Exception as e:
            st.error(f"解压ZIP失败: {str(e)}")
            return 0, []

    def get_files(self, user_id=None, form_id=None, record_id=None):
        """获取文件列表，可过滤"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        query = '''
            SELECT f.id, f.original_filename, f.file_size, f.mime_type, 
                   f.upload_time, f.description, u.username, f.form_id, f.record_id
            FROM files f
        ...
        if user_id is not None:
            conditions.append("f.uploaded_by = ?")
            params.append(user_id)
        if form_id is not None:
            conditions.append("f.form_id = ?")
            params.append(form_id)
        if record_id is not None:
            conditions.append("f.record_id = ?")
        ...
        for row in rows:
            files.append({
                "ID": row[0],
                "文件名": row[1],
                "大小(KB)": round(row[2] / 1024, 2) if row[2] else 0,
                "类型": row[3],
                "上传时间": row[4],
                "描述": row[5] or "",
                "上传者": row[6],
        ...
        return pd.DataFrame(files) if files else pd.DataFrame()
    ...
    def get_database_stats(self):
        ...
        login_log_count = cursor.fetchone()[0]
        cursor.execute("SELECT COUNT(*) FROM operation_logs")
        operation_log_count = cursor.fetchone()[0]
        cursor.execute("SELECT COUNT(*) FROM files")  # 新增
        file_count = cursor.fetchone()[0]
        cursor.execute("SELECT SUM(file_size) FROM files")  # 新增
        ...
        return {
            ...
            "total_file_size_mb": total_file_size / (1024 * 1024),  # 新增
            "db_size_kb": db_size
        }
...
def show_welcome_page():
    ...
    with col2:
        ...
        with col3:
            st.metric("数据库大小", f"{stats['db_size_kb']:.1f} KB")
        st.markdown('</div>', unsafe_allow_html=True)

    def _save_file_bytes(self, original_filename, file_bytes, uploaded_by, description="", form_id=None,
                         record_id=None):
        """
        内部方法：将文件字节保存到磁盘并插入数据库记录
        :param original_filename: 原始文件名（可包含路径）
...
def show_dashboard():
    ...
    with col4:
        if check_permission("view_files"):
            st.markdown(f"""
                <div class="stat-card">
                    <h3>{stats['file_count']}</h3>
                    <p>文件数量</p>
                    <small>{stats['total_file_size_mb']:.2f} MB</small>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
        ...
        if user_info:
            ...
            with col2:
                ...
                **角色:** {user_info[6]}  
                ...
                if 'department' in column_names and len(user_info) > 5:
                    user_info_text += f"**部门:** {user_info[5] or '未设置'}  \n"

                if 'last_login' in column_names and len(user_info) > 9:
                    user_info_text += f"**最后登录:** {user_info[9] or '从未登录'}  \n"

                if 'is_active' in column_names and len(user_info) > 8:
                    user_info_text += f"**账户状态:** {'✅ 活跃' if user_info[8] == 1 else '❌ 禁用'}"
    ...
    if check_permission("view_forms"):
        ...
        if forms:
            for form_id, form_name in forms[:3]:  # 显示前3个表单
...
def create_sample_data(form_id):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT form_config FROM form_definitions WHERE id=?", (form_id,))
    form_config = json.loads(cursor.fetchone()[0])
                        ...
                        try:
                            ...
                            if 'filters_applied' in st.session_state and st.session_state.filters_applied:
                                ...
                                if 'filtered_df' in st.session_state:
                                    ...
                                    # 创建映射：找到筛选后数据在原始数据中的索引
                                    # 由于筛选后的数据可能改变了顺序，我们需要通过内容匹配
                                    original_indices = []
...
def show_form_management():
            ...
            for form_id, form_name in forms:
                with st.expander(f"📋 {form_name} (ID: {form_id})"):
                    ...
                    with col1:
                        ...
                        conn = sqlite3.connect(system.db_path)
                        cursor = conn.cursor()
                        cursor.execute("SELECT form_config, created_at FROM form_definitions WHERE id=?", (form_id,))
                        form_config_data = cursor.fetchone()
                        conn.close()

                        form_config = json.loads(form_config_data[0])
                        created_at = form_config_data[1]
                        ...
                        if fields:
                            st.markdown("**字段预览:**")
                            for i, field in enumerate(fields[:3]):  # 只显示前3个字段
                                field_type = field.get('type', 'text')
                    ...
                    with col3:
                        ...
                        if st.button("✏️ 编辑", key=f"edit_{form_id}", use_container_width=True,
                            ...
                            # 注意：Streamlit不能直接切换tab，但我们可以通过JS或重新渲染来实现
                            # 这里我们显示消息，用户需要手动切换到"编辑表单"tab
                                ...
                                with col_confirm1:
                                    if st.button("✅ 确认删除", key=f"confirm_delete_yes_{form_id}",
                                        ...
                                        try:
                                            ...
                                            # 再删除表单定义
                                            cursor.execute("DELETE FROM form_definitions WHERE id=?", (form_id,))
                                            ...
                                            form_deleted = cursor.fetchone()[0]
                                            ...
                                            st.info(f"删除数据记录: {data_deleted} 条")
    ...
    with tab2:
        ...
        with st.form("create_form", clear_on_submit=True):
            ...
            # 使用form_submit_button替代普通的button
            ...
            if st.session_state.form_fields:
                ...
                with st.form("edit_fields_form"):
                    ...
                    for i, field in enumerate(st.session_state.form_fields):
                        ...
                        # 如果不是删除状态，则添加到编辑后的字段列表
                        if not delete_field and field_name:  # 确保字段名不为空
                        ...
                        if not edited_fields:
                            ...
                            if len(set(field_names)) != len(field_names):
                                ...
                                try:
                                    ...
                                    # 获取原始表单配置
                                    cursor.execute("SELECT form_config FROM form_definitions WHERE id=?",
                                                   (st.session_state.editing_form_id,))
                                    original_config = json.loads(cursor.fetchone()[0])

                                    # 创建新的表单配置
                                    new_config = {
                                        "form_name": original_config.get("form_name",
                                                                         st.session_state.editing_form_name),
                                        "fields": edited_fields
                                    }

                                    # 更新数据库
                                    cursor.execute("UPDATE form_definitions SET form_config = ? WHERE id = ?",
                                    ...
                                    # 更新现有数据 - 需要处理字段变化
                                    cursor.execute("SELECT id, data_json FROM form_data WHERE form_id=?",
                                                   (st.session_state.editing_form_id,))
                                    rows = cursor.fetchall()

                                    old_field_names = [field["name"] for field in original_config.get("fields", [])]
                                    ...
                                    for row_id, data_json in rows:
                                        try:
                                            ...
                                            # 更新数据库
                                            cursor.execute("UPDATE form_data SET data_json = ? WHERE id = ?",
                                                           (json.dumps(record, ensure_ascii=False), row_id))
                                        except:
                                            continue
                                    ...
                                    st.info(
                                        f"字段变更: 新增 {len(added_fields)} 个字段，删除 {len(removed_fields)} 个字段")
            ...
            if df1.empty or df2.empty:
                ...
                if not common_columns:
                    ...
                    if join_column:
                        ...
                        # 检查关联字段的数据质量
                        df1_non_null = df1[join_column].notna().sum()
                        df2_non_null = df2[join_column].notna().sum()

                        # 统计唯一值数量
                        df1_unique = df1[join_column].nunique()
                        df2_unique = df2[join_column].nunique()
                        ...
                        try:
                            ...
                            with col4:
                                st.metric(
                                    "总匹配率",
                                    f"{(matched_count / (total_form1_count + total_form2_count - matched_count)) * 100:.1f}%" if (
                                                                                                                                         total_form1_count + total_form2_count - matched_count) > 0 else "0%",
...
def show_file_management():
    ...
    with tab1:
        ...
        col1, col2 = st.columns(2)
        with col1:
            show_all = st.checkbox("显示所有用户文件", value=check_permission("manage_files"))
        with col2:
            form_filter = st.selectbox(
                "按表单筛选",
                ["全部"] + [f"{name} (ID:{id})" for id, name in system.get_forms()],
                key="file_form_filter"
            )

        if show_all and check_permission("manage_files"):
            files_df = system.get_files()
        else:
            files_df = system.get_files(user_id=user_id)

        if form_filter != "全部":
            form_id = int(form_filter.split("ID:")[1].rstrip(")"))
            files_df = files_df[files_df["关联表单ID"] == form_id]

        if files_df.empty:
            st.info("暂无文件")
        else:
            st.dataframe(files_df[["文件名", "大小(KB)", "类型", "上传时间", "描述", "上传者"]],
                         use_container_width=True, height=400)

            st.markdown("#### 操作")
            file_options = {row["文件名"]: row["ID"] for _, row in files_df.iterrows()}
            selected_file_name = st.selectbox("选择文件", list(file_options.keys()), key="file_select")
            if selected_file_name:
                file_id = file_options[selected_file_name]
                col1, col2, col3 = st.columns(3)
                with col1:
                    file_info = system.get_file_path(file_id)
                    if file_info:
                        ...
                        try:
                            ...
                            st.download_button(
                                label="📥 下载",
                                data=file_bytes,
                                file_name=os.path.basename(original_name),  # 下载时只取文件名
                            ...
                            st.error(f"文件读取失败: {e}")
                with col2:
                    file_owner = files_df[files_df["ID"] == file_id]["上传者"].iloc[0]
                    if check_permission("manage_files") or file_owner == current_user['username']:
                        if st.button("🗑️ 删除", use_container_width=True):
                            if system.delete_file(file_id, user_id):
                                st.success("文件已删除")
                                st.rerun()
                    else:
    ...
    with tab2:
        # 原有的单个文件上传功能（保持不变）
        st.markdown("### 上传单个文件")
        if not check_permission("manage_files"):
            st.warning("您没有上传文件的权限")
        else:
            uploaded_file = st.file_uploader("选择文件", type=None, accept_multiple_files=False, key="single_upload")
            if uploaded_file is not None:
                description = st.text_input("文件描述（可选）", key="single_desc")
                forms = system.get_forms()
                form_options = {f"{name} (ID:{id})": id for id, name in forms}
                form_options["无关联"] = None
                selected_form = st.selectbox("关联表单（可选）", list(form_options.keys()), key="single_form")
                form_id = form_options[selected_form]

                if st.button("上传", key="single_upload_btn"):
                    with st.spinner("上传中..."):
                        file_id = system.upload_file(
                            uploaded_file,
                            uploaded_by=user_id,
    ...
    with tab3:
        # 原有的压缩包上传功能（支持ZIP/RAR）
        st.markdown("### 上传文件夹（ZIP/RAR压缩包）")
        st.markdown("请将文件夹打包为ZIP或RAR格式上传，系统将解压并保留文件夹结构。")
        if not check_permission("manage_files"):
            st.warning("您没有上传文件的权限")
        else:
            ...
            if uploaded_archive is not None:
                description = st.text_input("文件夹描述（可选）", key="archive_desc")
                forms = system.get_forms()
                form_options = {f"{name} (ID:{id})": id for id, name in forms}
                form_options["无关联"] = None
                selected_form = st.selectbox("关联表单（可选）", list(form_options.keys()), key="archive_form")
                form_id = form_options[selected_form]

                if st.button("上传并解压", key="archive_upload_btn"):
                    with st.spinner("正在处理压缩包，请稍候..."):
                        success_count, file_ids = system.upload_archive(
                            archive_file=uploaded_archive,
                            uploaded_by=user_id,
                            description=description,
                            form_id=form_id
                        )
                    if success_count > 0:
                        ...
                        with st.expander("查看上传的文件名列表"):
                            st.write(file_ids)
                        st.rerun()
                    else:
                        st.error("上传失败，请检查压缩包是否有效")

    with tab4:
        # 新增：多文件上传（可指定基础路径）
        st.markdown("### 多文件上传（可指定基础路径）")
        st.info("由于浏览器限制，直接选择文件夹会丢失结构。您可以通过下方输入一个基础路径，上传的所有文件将保存到该路径下。如需保留文件夹结构，请使用ZIP/RAR上传。")
        if not check_permission("manage_files"):
            st.warning("您没有上传文件的权限")
        else:
            ...
            if uploaded_files:
                base_path = st.text_input("基础路径（可选，例如：项目资料/合同）", key="base_path").strip()
                if base_path and not base_path.endswith('/'):
                    base_path += '/'  # 确保末尾有斜杠
                description = st.text_input("文件描述（可选）", key="multi_desc")
                forms = system.get_forms()
                form_options = {f"{name} (ID:{id})": id for id, name in forms}
                form_options["无关联"] = None
                selected_form = st.selectbox("关联表单（可选）", list(form_options.keys()), key="multi_form")
                form_id = form_options[selected_form]

                if st.button("开始上传", key="multi_upload_btn"):
                    ...
                    for i, uploaded_file in enumerate(uploaded_files):
                        # 构造原始文件名：基础路径 + 原文件名
                        original_name = base_path + uploaded_file.name if base_path else uploaded_file.name
                        file_bytes = uploaded_file.getbuffer()
                        file_id = system._save_file_bytes(
                            original_filename=original_name,
                            file_bytes=file_bytes,
                            uploaded_by=user_id,
                            description=description,
                        ...
                        status_text.text(f"已上传 {i+1}/{total}")
                    if success_count > 0:
                        st.success(f"✅ 成功上传 {success_count} 个文件！")
                        with st.expander("查看上传的文件名列表"):
                            st.write(file_ids)
                ...
                if selected_user_key:
                    ...
                    if user:
                        with st.form("edit_user_form"):
                            ...
                            if permissions_json:
                                try:
                                    current_permissions = json.loads(permissions_json)
                                except:
                                    current_permissions = []
                            else:
                                current_permissions = []
...
def show_database_management():
    ...
    with col1:
        if st.button("💾 备份数据库", type="primary", use_container_width=True):
            ...
            backup_path = os.path.join(backup_dir, f"企业帮扶台账_备份_{timestamp}.db")
            ...
            if st.button("清理旧日志", use_container_width=True):
                try:
                    ...
                    cursor.execute('''
                        ...
                        WHERE julianday('now') - julianday(operation_time) > ?
                    ''', (days_to_keep,))
                    ...
                    cursor.execute('''
                        ...
                        WHERE julianday('now') - julianday(login_time) > ?
                    ''', (days_to_keep,))

                    deleted_login_logs = cursor.rowcount

                    conn.commit()
                    conn.close()

                    st.success(f"清理完成！删除操作日志: {deleted_logs} 条，登录日志: {deleted_login_logs} 条")
                    ...
                    system.log_operation(
                        ...
                        details={"days_to_keep": days_to_keep, "deleted_logs": deleted_logs,
...
def show_system_info():
    ...
    try:
        ...
        # 检查表状态
        tables = ["users", "form_definitions", "form_data", "role_permissions", "login_logs", "operation_logs"]
        table_status = {}
        ...
        for table in tables:
            try:
                ...
                table_status[table] = {"status": "正常", "记录数": count}
            except:
                table_status[table] = {"status": "异常", "记录数": 0}
...
def main():
    ...
    with st.sidebar:
        ...
        if not st.session_state.logged_in:
            ...
            if check_permission("view_files"):  # 新增文件管理菜单
                menu_options.append("📁 文件管理")
